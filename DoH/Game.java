...
//      System.out.println(gb.state);
        ...
        if (gb.getPlayer(gb.phase).getClass() == new HumanPlayer().getClass()) {
            if (gb.state == GameBoard.State.SELECT_UNIT) {
                if (gb.isUnitOn(gb.map.cursor)) {
                    ...
                    g.fillRect(left, top, 120, 70);
                    g.setColor(Color.BLACK);
                    g.drawString(uoi.job.name(), left + 2, top + 30);
                    g.drawString(uoi.currHP + "/" + uoi.maxHP, left + 8, top + 50);
                    g.drawString("STR: " + uoi.str, left + 75, top + 20);
                    g.drawString("DEF: " + uoi.def, left + 75, top + 40);
                    g.drawString("SPD: " + uoi.speed, left + 75, top + 60);
                }
    ...
    private void drawTerrainInfo(Graphics g) {
        ...
        int left = 200;
        g.setColor(new Color(250, 250, 250, 120));
        g.fillRect(left, top, (gb.map.width-1)*W - 200, 70);
        g.setColor(Color.BLACK);
        g.drawString(gb.map.getCell(gb.map.cursor).type.toString(), left + 2, top + 20);
        g.drawString("Avoid Bonus: " + gb.map.getCell(gb.map.cursor).type.avoid, left + 2, top + 40);
        g.drawString("Defense Bonus: " + gb.map.getCell(gb.map.cursor).type.defense, left + 2, top + 60);
